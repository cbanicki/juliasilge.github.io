...
The values for Monday through Friday are `r round(mean(myTable[c(2:5)])/mean(myTable[c(1,6,7)]), digits = 3)` higher than the other days, on average, close to 5/4. Let's see if the chi-squared test says that my pattern of tweets is consistent with tweeting 1.25 times as often (5/4) on Monday through Thursday as on Friday through Sunday.
...
It turns out it's about `r round(mean(is.na(tweets$timeonly)), digits = 3)*100`%, so for my tweets vs. time graph, we will actually have not-quite-10,000 tweets. To make the graph, let's convert the new `timeonly` column to a date-time object and make a histogram. We can use a scaling function from the `scales` library to scale and label the x-axis.
...
Only `r round(mean(!is.na(tweets$retweeted_status_id)), digits = 3)*100`% of my tweets are retweets of others. The `scale_x_discrete` function is a nice, useful thing for relabeling the levels of a categorical variable for plotting. Before I applied this function, the labels just said "FALSE" and "TRUE".
...
So `r round(mean(!is.na(tweets$in_reply_to_status_id)), digits = 3)*100`% of my tweets are in reply to another person's tweets, quite a bit higher than the other categories we've looked at. I am a fairly social user of the social media, it appears.
